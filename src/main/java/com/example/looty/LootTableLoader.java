...
public class LootTableLoader {
        ...
        try (Reader reader = Files.newBufferedReader(Paths.get(lootConfigFile.toURI()))) {
            ...
            for (Map.Entry<String, JsonElement> entry : root.entrySet()) {
                ...
                for (JsonElement e : lootArray) {
                    ...
                    if (itemRes == null || !ForgeRegistries.ITEMS.containsKey(itemRes)) {
                        LOGGER.warn("Invalid or unknown item '{}'. Skipping.", itemId);
                        continue;
                    }

                    Item item = ForgeRegistries.ITEMS.getValue(itemRes);
                    if (item == null || item == ItemStack.EMPTY.getItem()) {
                        LOGGER.warn("Resolved item '{}' is null or empty. Skipping.", itemId);
                        continue;
                    }
    ...
    public static List<ItemStack> getRandomizedLoot(String rarity) {
        ...
        while (result.size() < config.minItems() && !pool.isEmpty()) {
            ...
            if (Config.enableDuplicateItems || !alreadyIncluded.contains(id)) {
    ...
    public static boolean hasRarity(String rarity) {
        return lootTables.containsKey(rarity);
    }
